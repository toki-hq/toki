...
- **Named channels** + **duplex mode** — per-frequency settings in the Channels view: a human-readable name, and **Half** (single-floor walkie-talkie, the default) vs **Full** duplex (everyone can talk at once). Full-duplex is gated by a **Settings toggle** (off by default) — while off every channel is half-duplex, the mode can't be set, and duplex controls/indicators are hidden everywhere. Mode changes hot-apply to a live room and push to its occupants.
