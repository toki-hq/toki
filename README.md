...
- **Frequencies as channels** — rooms are addressed by frequency string (e.g. `"446.05"`). The band is 446.00–448.00 MHz in 0.05 MHz steps (41 discrete channels); each is an independent room with its own member list. A channel is **half-duplex** by default (a single PTT floor — classic walkie-talkie), or the operator can switch it to **full-duplex** (no floor: several members transmit at once and each client mixes the streams). PTT is kept in both modes; on a full-duplex channel a press transmits immediately rather than requesting the floor.
...
- **Operator actions** — kick, move to frequency, rename callsign, grant/revoke global broadcast.
...
- **Duplex mode** — a per-frequency setting in the Channels view: **Half** (single-floor walkie-talkie, the default) vs **Full** duplex (everyone can talk at once and clients mix the streams). Full-duplex is gated by a **Settings toggle** (off by default) — while off every channel is half-duplex, the mode can't be set, and duplex controls/indicators are hidden everywhere. Mode changes hot-apply to a live room and push to its occupants.
- **Global broadcast** — an admin-granted, session-scoped capability that lets one designated client transmit to every occupied frequency simultaneously. When the broadcaster keys their broadcast PTT, the floor is seized on all channels, any current talker is cut off, and local talkers are blocked for the duration; listeners hear a distinct three-step falling roger (C6 → G5 → C5). Pierces all mutes. Only one holder at a time; clears automatically on disconnect. Grant/revoke from the Channels view member context menu; recorded in the audit log as `global_broadcast`.
- **Runtime config** — server name, `max_peers`, idle-kick timeout, the named-channels toggle, the **unique-callsigns** toggle, the **full-duplex** toggle, **voice quality** (Raw PCM / Low / Standard / High Opus), and the gRPC server password, all hot-reloaded without a restart (codec changes apply to clients on their next connect).
- **Unique callsigns** — on by default: a register or admin rename onto a callsign another connected member already holds is refused (case-insensitive — `ECHO-1` ≈ `echo-1`), so there's only ever one of each on the server. A name frees up the instant its holder disconnects; a keypair client reconnecting keeps its own name. Toggle off to allow duplicates.
...
- **Global hotkeys** (Settings) — bind PTT, the M1–M4 recalls, and tune up/down to *any* peripheral: keyboard, mouse, game controller/joystick, Elgato Stream Deck, or generic USB HID. PTT (default backtick) also takes an optional secondary/fallback binding. A separate **broadcast PTT** binding (Settings → Controls, or `broadcast_ptt` in `config.toml`) activates global broadcast when the admin has granted the capability. Keyboard/mouse stay passthrough; other devices are capture-only.
...
- **VOX (voice-activated transmit)** — on full-duplex channels, an opt-in gate (Settings → Audio → VOICE ACTIV.) opens the mic automatically when speech is detected, with a tunable sensitivity slider and a ~300 ms hangover so brief word-gaps don't cut out. PTT key overrides VOX when held; ignored on half-duplex channels.
- **Roger beeps** — selectable take-/clear-floor tone presets, with a fixed two-tone cue for priority traffic. Suppressed on full-duplex channels (no single floor to announce); the broadcast roger and preempted bump still play.
