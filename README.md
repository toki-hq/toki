...
- **Channel mute** — silence a *whole frequency*: while muted, no one tuned there can transmit (moving to another channel restores it instantly). Same speak-gate as member mute, persisted across restarts and occupancy; muting drops any in-progress floor. Toggle per-channel from the panel; a MUTED badge flags it (occupied or not).
