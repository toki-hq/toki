...
- **Runtime config** — server name, `max_peers`, idle-kick timeout, the named-channels toggle, the **unique-callsigns** toggle, **voice quality** (Raw PCM / Low / Standard / High Opus), and the gRPC server password, all hot-reloaded without a restart (codec changes apply to clients on their next connect).
