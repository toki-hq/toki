...
- **Identity bans**: admins ban an identity (optionally its machine hash too — a wiped config stays banned) from the panel; the session is kicked and future registers are rejected with the operator's reason. Bans persist in the admin DB and are reviewed/lifted in the panel's Bans view; rejected attempts land in the audit log.
